...
def prescriptions(appointment_id):
    ...
    # Get appointment with verification
        ...
        try:
            ...
            for i in range(1, medicine_count + 1):
                ...
                if name and dosage and frequency:
                    ...
                    medicines.append(f"{name} {dosage} - {frequency} (Times: {morning}-{afternoon}-{evening}, {meal} meal)")
            
            instructions = request.form.get('instructions', '').strip()
            
            # Save to database
            existing = conn.execute('SELECT id FROM prescriptions WHERE appointment_id = ?', 
            ...
            if existing:
                ...
                ''', (diagnosis, '\n'.join(medicines), instructions, appointment_id))
            else:
                conn.execute('''
                    INSERT INTO prescriptions 
                    (appointment_id, diagnosis, medicines, instructions, hospital_id)
                    VALUES (?, ?, ?, ?, ?)
                ''', (appointment_id, diagnosis, '\n'.join(medicines), instructions, session['hospital_id']))
            
            conn.commit()
            
            # Check if we need to redirect to print
...
def print_prescription(appointment_id):
    ...
    try:
        ...
        prescription_data = conn.execute('''
            ...
            WHERE p.appointment_id = ? AND p.hospital_id = ?
        ''', (appointment_id, session['hospital_id'])).fetchone()
        
        if not prescription_data:
            flash('Prescription not found! Please save the prescription first.', 'danger')
            return redirect(url_for('prescriptions', appointment_id=appointment_id))
        
        # Process prescription data for display
        prescription = dict(prescription_data)
        # Add medicines parsing logic here...
        
        return render_template('doctor/print_prescription.html',
                            prescription=prescription)
        
    except Exception as e:
        flash(f'Error loading prescription: {str(e)}', 'danger')
        return redirect(url_for('doctor_appointments'))
    finally:
        conn.close()
